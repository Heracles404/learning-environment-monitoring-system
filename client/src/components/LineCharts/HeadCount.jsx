    ...
    const fetchData = async () => {
      try {
        ...
        const transformedData = [{
          id: "Headcount",
          color: tokens("dark").blueAccent[500], // Customize color
          data: []
        }];

        // Loop through the fetched data and extract headcount values
        fetchedData.forEach(item => {
          const dataPoint = {
            x: new Date(item.date + ' ' + item.time).toISOString(), // Combine date and time for x-axis
            y: item.headCount // Access the headCount property
          };
  ...
  return (
    <div style={{ height: '400px', width: '100%' }}> {/* Set a fixed height for visibility */}
      {data.length > 0 ? (
        ...
        <p>No data available for Headcount.</p>
      )}
    </div>
  );
};
