...
const LightingChart = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        console.log("Fetching lighting data...");
        ...
        const transformedData = [{
          id: "Lighting",
          color: colors.yellowAccent?.[500] || '#FFD700', // Fallback color
          data: []
        }];

        // Loop through the fetched data and extract lighting values
        fetchedData.forEach(item => {
          const dataPoint = {
            x: new Date(item.date + ' ' + item.time).toISOString(), // Combine date and time for x-axis
            y: item.lighting // Access the lighting property
          };
          transformedData[0].data.push(dataPoint); // Push data point to the existing entry
        });

        console.log("Transformed lighting data for chart:", transformedData); // Log the transformed data for the chart
      ...
      {data.length > 0 ? (
        <ResponsiveLine
          ...
          axisLeft={{
            ...
            tickPadding: 5,
            tickRotation: 0,
            legend: "Lighting (Lux)", // Adjust the legend as needed
            legendOffset: -40,
            legendPosition: "middle",
          }}
          enableGridX={false}
        ...
        <p>No data available for Lighting.</p>
      )}
    </div>
  );
};
