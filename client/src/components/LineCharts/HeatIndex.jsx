...
const HeatIndexChart = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // console.log("Fetching heat index data...");
        ...
        const transformedData = [{
          id: "Heat Index",
          color: tokens("dark").redAccent[500], // Customize color
          data: []
        }];

        // Loop through the fetched data and extract heat index values
        fetchedData.forEach(item => {
          const dataPoint = {
            x: new Date(item.date + ' ' + item.time).toISOString(), // Combine date and time for x-axis
            y: item.heatIndex // Access the heatIndex property
          };
          transformedData[0].data.push(dataPoint); // Push data point to the existing entry
        });

        // console.log("Transformed heat index data for chart:", transformedData); // Log the transformed data for the chart
  ...
  return (
    <div style={{ height: '400px', width: '100%' }}> {/* Set a fixed height for visibility */}
      {data.length > 0 ? (
        ...
        <p>No data available for Heat Index.</p>
      )}
    </div>
  );
};
